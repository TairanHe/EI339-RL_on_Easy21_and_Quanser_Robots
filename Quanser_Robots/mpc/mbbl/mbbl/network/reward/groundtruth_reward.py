...
class reward_network(base_reward_network):
    ...
    def pred(self, data_dict):
        ...
        for i_data in range(len(data_dict['action'])):
            ...
            if self.quanser:
                i_reward = self._env.rwd(
                    {key: data_dict[key][i_data] for key in key_list}["start_state"], {key: data_dict[key][i_data] for key in key_list}["action"]
                )
            else:
