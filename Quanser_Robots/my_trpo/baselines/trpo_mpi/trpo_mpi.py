...
def traj_segment_generator(pi, env, horizon, stochastic):
    ...
    print(ac)
    print("standard action shape:", ac.shape)
    ...
    while True:
        ...
        if env.unwrapped.spec.id in ["Qube-100-v0", "BallBalancerSim-v0", "CartpoleSwingShort-v0"]:
...
def learn(*,
    ...
    **network_kwargs        keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    while True:
        ...
        rew_avg = np.mean(rewbuffer)
        rew_std = np.std(rewbuffer)
        logger.record_tabular("EpLenMean", np.mean(lenbuffer))
        logger.record_tabular("EpRewMean", rew_avg)
        logger.record_tabular("EpRewStd", rew_std)
        ...
        if ckpt_dir is not None:
            ...
            if rew_avg >= best:
                best = rew_avg
                pi.save(fname + "_" + str(save_ind))
                pi.save(fname + "_best")
            save_ind = (save_ind + 1) % max_save
            if (iters_so_far + 1) % 1000 == 0:
                pi.save(fname + "_iter_" + str(iters_so_far + 1))
