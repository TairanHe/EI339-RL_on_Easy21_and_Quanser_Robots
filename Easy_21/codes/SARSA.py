...
class SARSA:
    ...
    def _update_q_table(self, state_, action_, reward_, next_state_):
        ...
        q_target = reward_ if game_end == 1 else reward_ + self.gamma * self.q_table[next_state_][self.sarsa_action]
    ...
    def run_one_step(self, state):
        ...
        self.sarsa_action = self.choose_action(next_state)
    ...
    def run_one_episode(self):
        ...
        self.sarsa_action = self.choose_action(state)
    ...
    def save_fig(self, fig_path):
        ...
        plot_Q(self.q_table, save_path=(fig_path + "SARSA_" + self.name + "_value.pdf"))

    def save_result(self, log_path):
        """ Save the logs in the given path. """
        with open(log_path + "SARSA_" + self.name + "_value.txt", "w") as f:
            f.write(str(self.q_table))
        try:
            ART = np.array(self.average_reward_trace, dtype=np.float16)
            ART_name = log_path + "SARSA_" + self.name + "_ave_reward_{}.txt".format(len(ART))
            np.savetxt(ART_name, ART, fmt="%.4f", delimiter=",")
            RMSE = np.array(self.rmse_trace, dtype=np.float16)
            RMSE_name = log_path + "SARSA_" + self.name + "_rmse_{}.txt".format(len(RMSE))
...
def main():
    ...
    for name, param in PARAM_DICT.items():
        print("#" * 5, name, "#" * 5)
        model = SARSA(env, episode_num=EPISODE_NUM,
    ...
    plot_learning_curve(ave_rewards[:5], rmses[:5], names[:5],
                        save_path=fig_path + "SARSA_different_alpha_CMP.pdf", show=False)
    plot_learning_curve(ave_rewards[5:], rmses[5:], names[5:],
                        save_path=fig_path + "SARSA_different_epsilon_CMP.pdf", show=False)
